...
public sealed class DxgiCapture : IDisposable
{
    ...
    public DxgiCapture(IntPtr hMonitor, int targetFps = 60)
    {
        ...
        for (uint ai = 0; ; ai++)
        {
            ...
            for (uint oi = 0; ; oi++)
            {
                ...
                if (desc.Monitor == hMonitor)
                {
                    ...
                    targetOutput = output;
                    Console.WriteLine($"[DXGI] Found output: {desc.DeviceName} on {adapter.Description.Description}");
                    break;
                }
                output.Dispose();
            }
        ...
        Console.WriteLine($"[DXGI] Output size: {_width}x{_height}");
        ...
        Console.WriteLine("[DXGI] Desktop duplication created");
    ...
    public void Start()
    {
        ...
        Console.WriteLine("[DXGI] Capture started");
    }
    ...
    public void Stop()
    {
        ...
        Console.WriteLine("[DXGI] Capture stopped");
    }
            ...
            try
            {
                ...
                if (result.Success && desktopResource != null)
                {
                    try
                    {
                        ...
                        {
                            Console.WriteLine($"[DXGI] Frame #{frameCount}: {_width}x{_height}");
                        }
                        
                        // Invoke callback
                        OnFrame?.Invoke(_buffer, _width, _height, _stride);
                    }
                    finally
                    {
                ...
                {
                    Console.WriteLine("[DXGI] Access lost - desktop mode changed?");
                    Thread.Sleep(500);
                }
            }
            catch (Exception ex)
