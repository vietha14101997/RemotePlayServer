...
public unsafe class QsvNativeWrapper : ITextureEncoder
{
    ...
    public static bool IsAvailable()
    {
        ...
        {
            Console.WriteLine("[QsvNativeWrapper] QsvWrapper.dll not found");
            return false;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[QsvNativeWrapper] IsAvailable check failed: {ex.Message}");
            return false;
        }
    }
    ...
    public bool Initialize(int width, int height, int fps, int bitrate, ID3D11Device device)
    {
        ...
        try
        {
            ...
            Console.WriteLine($"[QsvNativeWrapper] Initializing {width}x{height} @ {fps}fps, {bitrate}kbps");
            ...
            if (result != QSV_WRAPPER_OK)
            {
                ...
                Console.WriteLine($"[QsvNativeWrapper] QsvCreateEncoder failed: {error}");
                _handle = IntPtr.Zero;
                return false;
            }
            ...
            if (result != QSV_WRAPPER_OK)
            {
                Console.WriteLine("[QsvNativeWrapper] Failed to set callback");
                Cleanup();
                return false;
            }

            Console.WriteLine("[QsvNativeWrapper] Initialized successfully (zero-copy enabled)");
            return true;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[QsvNativeWrapper] Initialize exception: {ex.Message}");
            Cleanup();
            return false;
        }
    }
    ...
    public bool EncodeTexture(ID3D11Texture2D texture, bool forceKeyframe = false)
    {
        ...
        {
            Console.WriteLine($"[QsvNativeWrapper] EncodeTexture exception: {ex.Message}");
            return false;
        }
    }
    ...
    public bool SetBitrate(int bitrateKbps)
    {
        ...
        try
        {
            ...
            if (result == QSV_WRAPPER_OK)
            {
                ...
                Console.WriteLine($"[QsvNativeWrapper] Bitrate changed to {bitrateKbps}kbps");
                return true;
            }
            else
            {
                string error = GetLastError();
                Console.WriteLine($"[QsvNativeWrapper] SetBitrate failed: {error}");
                return false;
            }
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[QsvNativeWrapper] SetBitrate exception: {ex.Message}");
            return false;
        }
    }
    ...
    private void NativeCallback(IntPtr data, uint size, long pts, int isKeyFrame, IntPtr userData)
    {
        ...
        {
            Console.WriteLine($"[QsvNativeWrapper] Callback exception: {ex.Message}");
        }
    }
    ...
    private void Cleanup()
    {
        ...
        {
            Console.WriteLine($"[QsvNativeWrapper] Cleanup exception: {ex.Message}");
        }
    }
    ...
    public void Dispose()
    {
        ...
        Cleanup();
        Console.WriteLine("[QsvNativeWrapper] Disposed");
    }
