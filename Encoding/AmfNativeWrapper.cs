...
public unsafe class AmfNativeWrapper : ITextureEncoder
{
    ...
    public static bool IsAvailable()
    {
        ...
        {
            Console.WriteLine("[AmfNativeWrapper] AmfWrapper.dll not found");
            return false;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[AmfNativeWrapper] IsAvailable check failed: {ex.Message}");
            return false;
        }
    }
    ...
    public bool Initialize(int width, int height, int fps, int bitrate, ID3D11Device device)
    {
        ...
        try
        {
            ...
            Console.WriteLine($"[AmfNativeWrapper] Initializing {width}x{height} @ {fps}fps, {bitrate}kbps");
            ...
            if (result != AMF_WRAPPER_OK)
            {
                ...
                Console.WriteLine($"[AmfNativeWrapper] AmfCreateEncoder failed: {error}");
                _handle = IntPtr.Zero;
                return false;
            }
            ...
            Console.WriteLine("[AmfNativeWrapper] Initialized successfully (zero-copy enabled)");
            return true;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[AmfNativeWrapper] Initialize exception: {ex.Message}");
            Cleanup();
            return false;
        }
    }
    ...
    public bool EncodeTexture(ID3D11Texture2D texture, bool forceKeyframe = false)
    {
        ...
        {
            Console.WriteLine($"[AmfNativeWrapper] EncodeTexture exception: {ex.Message}");
            return false;
        }
    }
    ...
    public bool EncodeNV12Bytes(byte[] nv12Data, bool forceKeyframe = false)
    {
        ...
        {
            Console.WriteLine($"[AmfNativeWrapper] EncodeNV12Bytes exception: {ex.Message}");
            return false;
        }
    }
    ...
    private void NativeCallback(IntPtr data, uint size, long pts, int isKeyFrame, IntPtr userData)
    {
        ...
        {
            Console.WriteLine($"[AmfNativeWrapper] Callback exception: {ex.Message}");
        }
    }
    ...
    private void Cleanup()
    {
        ...
        {
            Console.WriteLine($"[AmfNativeWrapper] Cleanup exception: {ex.Message}");
        }
    }
    ...
    public void Dispose()
    {
        ...
        Cleanup();
        Console.WriteLine("[AmfNativeWrapper] Disposed");
    }
