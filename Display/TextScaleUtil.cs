...
static class TextScaleUtil
{
    ...
    public static void EnhancedBroadcastAndRefresh()
    {
        try
        {
            ...
            Console.WriteLine("[TextScale] Enhanced broadcast refresh completed.");
        }
        catch (Exception ex)
        {
            Console.WriteLine("[TextScale] Enhanced broadcast failed: " + ex.Message);
        }
    }
    ...
    public static bool SetPerMonitorTextScale(string monitorName, int percent)
    {
        try
        {
            ...
            Console.WriteLine($"[TextScale] Setting {percent}% (DPI: {dpi}) for monitor: {monitorName}");
            ...
            try
            {
                ...
                Console.WriteLine("[TextScale] Process DPI awareness enabled");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[TextScale] DPI awareness setup failed: {ex.Message}");
            }
            ...
            try
            {
                ...
                Console.WriteLine($"[TextScale] PerMonitorSettings registry written for {monitorName}");
                registrySuccess = true;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[TextScale] PerMonitorSettings registry failed: {ex.Message}");
            }
            ...
            {
                Console.WriteLine($"[TextScale] Desktop registry failed: {ex.Message}");
            }
            ...
            try
            {
                ...
                if (rk3 != null)
                {
                    ...
                    Console.WriteLine($"[TextScale] GraphicsDrivers registry written for {monitorName}");
                    registrySuccess = true;
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[TextScale] GraphicsDrivers registry failed (may be normal): {ex.Message}");
            }
            ...
            try
            {
                ...
                Console.WriteLine("[TextScale] DPI awareness context set to Per-Monitor V2");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[TextScale] DPI awareness context setup failed: {ex.Message}");
            }
            ...
            try
            {
                ...
                Console.WriteLine("[TextScale] Display change broadcast sent");
            ...
            {
                Console.WriteLine($"[TextScale] Broadcast failed: {ex.Message}");
            }
            ...
            if (success)
            {
                Console.WriteLine($"[TextScale] ✓ Per-monitor {percent}% set successfully for {monitorName}");
                Console.WriteLine($"[TextScale] Confirmed DPI: {effectiveDpi} (target: {dpi})");
            }
            else
            {
                Console.WriteLine($"[TextScale] ⚠ Current DPI: {effectiveDpi}, Expected: {dpi}");
                Console.WriteLine($"[TextScale] DPI check 1: {dpiCheck1}, DPI check 2: {dpiCheck2}");
                Console.WriteLine("[TextScale] This may require Explorer restart for full effect");
                ...
                try
                {
                    ...
                    Console.WriteLine("[TextScale] Final system-wide update initiated");
                }
                catch (Exception ex2)
                {
                    Console.WriteLine($"[TextScale] Final attempt failed: {ex2.Message}");
                }
            }
        ...
        {
            Console.WriteLine($"[TextScale] Per-monitor scaling failed: {ex.Message}");
            return false;
        }
    }
    ...
    public static int GetMonitorDpi(string monitorName)
    {
        try
        {
            ...
            {
                Console.WriteLine($"[TextScale] Registry DPI for {monitorName}: {dpiFromReg}");
                return dpiFromReg;
            }
            ...
            try
            {
                ...
                if (targetMonitor.hmon != IntPtr.Zero)
                {
                    ...
                    Console.WriteLine($"[TextScale] Actual DPI for {monitorName}: {actualDpi}");
                    return actualDpi;
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[TextScale] Cannot get actual DPI: {ex.Message}");
            }
            
            // Method 3: Fallback to system DPI
            int systemDpi = GetSystemDpi();
            Console.WriteLine($"[TextScale] Using system DPI for {monitorName}: {systemDpi}");
            return systemDpi;
        }
        catch { return 96; }
    }
    ...
    public static bool RestorePerMonitorTextScale(string monitorName, int originalPercent)
    {
        try
        {
            ...
            if (originalPercent == 100) // 100% means remove custom DPI
            {
                ...
                Console.WriteLine($"[TextScale] Removed custom DPI for {monitorName}");
            }
            else
            {
                rk.SetValue("LogPixels", dpi, RegistryValueKind.DWord);
                rk.SetValue("DpiScaling", 1, RegistryValueKind.DWord);
                Console.WriteLine($"[TextScale] Restored {originalPercent}% for {monitorName}");
            }
        ...
        {
            Console.WriteLine($"[TextScale] Per-monitor restore failed: {ex.Message}");
            return false;
        }
    }
