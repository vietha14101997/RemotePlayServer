...
static class DpiScalingHelper
{
    ...
    public static DpiScalingInfo GetDpiScalingInfo(LUID adapterId, uint sourceId)
    {
        ...
        {
            Console.WriteLine($"[DpiScaling] DisplayConfigGetDeviceInfo failed: {result}");
            return info;
        }
    ...
    public static bool SetDpiScaling(LUID adapterId, uint sourceId, uint dpiPercent)
    {
        ...
        {
            Console.WriteLine("[DpiScaling] Cannot get current DPI info");
            return false;
        }

        if (dpiPercent == currentInfo.Current)
        {
            Console.WriteLine($"[DpiScaling] Already at {dpiPercent}%");
            return true;
        }
        ...
        {
            Console.WriteLine($"[DpiScaling] Invalid DPI value: {dpiPercent}");
            return false;
        }
        ...
        if (result != ERROR_SUCCESS)
        {
            Console.WriteLine($"[DpiScaling] DisplayConfigSetDeviceInfo failed: {result}");
            return false;
        }

        Console.WriteLine($"[DpiScaling] ✓ Set DPI to {dpiPercent}%");
        return true;
    }
    ...
    public static bool SetAllMonitorsDpiScaling(uint dpiPercent)
    {
        ...
        if (result == null)
        {
            Console.WriteLine("[DpiScaling] Failed to get display config");
            return false;
        }
        ...
        Console.WriteLine($"[DpiScaling] Setting {dpiPercent}% for {paths.Length} monitor(s)...");

        foreach (var path in paths)
        {
            var info = GetDpiScalingInfo(path.sourceInfo.adapterId, path.sourceInfo.id);
            Console.WriteLine($"[DpiScaling] Monitor {path.sourceInfo.id}: current={info.Current}%, recommended={info.Recommended}%, max={info.Maximum}%");
