...
public static class GpuVendorDetector
{
    ...
    public static GpuVendor DetectPrimaryGpuVendor()
    {
        try
        {
            ...
            for (uint i = 0; ; i++)
            {
                ...
                try
                {
                    ...
                    {
                        Logger.Info($"[GpuVendorDetector] Detected NVIDIA GPU: {desc.Description}");
                        return GpuVendor.NVIDIA;
                    }
                    else if (vendorId == 0x1002 || vendorId == 0x1022) // AMD/ATI
                    {
                        Logger.Info($"[GpuVendorDetector] Detected AMD GPU: {desc.Description}");
                        return GpuVendor.AMD;
                    }
                    else if (vendorId == 0x8086) // Intel
                    {
                        Logger.Info($"[GpuVendorDetector] Detected Intel GPU: {desc.Description}");
        ...
        {
            Logger.Error($"[GpuVendorDetector] Detection failed: {ex.Message}");
        }
    ...
    public static IntelDriverInfo GetIntelDriverInfo()
    {
        ...
        try
        {
            ...
            for (uint i = 0; ; i++)
            {
                ...
                try
                {
                    ...
                    Logger.Info($"[GpuVendorDetector] Intel driver: {info.DriverVersionString}, recommended: {info.RecommendedEncoder}");
                    return info;
                }
                finally
                {
        ...
        {
            Logger.Error($"[GpuVendorDetector] Intel driver detection failed: {ex.Message}");
    ...
    private static Version? GetIntelDriverVersionFromRegistry(string gpuName)
    {
        ...
        {
            Logger.Error($"[GpuVendorDetector] Registry query failed: {ex.Message}");
        }
