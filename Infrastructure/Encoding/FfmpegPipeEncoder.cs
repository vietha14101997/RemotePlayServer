...
internal sealed class FfmpegPipeEncoder : IDisposable
{
    ...
    static GpuVendor DetectGpuVendor()
    {
        ...
        {
            Logger.Error($"[FFMPEG] GPU detection error: {ex.Message}");
        }
        Logger.Info("[FFMPEG] GPU vendor: Unknown, will probe all encoders");
    ...
    bool ProbeFilter(string filterName)
    {
        ...
        try
        {
            ...
            Logger.Info($"[FFMPEG][probe filter] {filterName} = {found}");
            return found;
        }
        catch (Exception ex)
        {
            Logger.Error($"[FFMPEG][probe filter] {filterName} EX: {ex.Message}");
    ...
    bool ProbeEncoder(string encName)
    {
        ...
        try
        {
            ...
            Logger.Info($"[FFMPEG][probe {encName}] exit={p.ExitCode}");
            if (!string.IsNullOrWhiteSpace(err)) Logger.Debug(err);

            return p.ExitCode == 0;
        }
        catch (Exception ex)
        {
            Logger.Error($"[FFMPEG][probe {encName}] EX: {ex.Message}");
            return false;
        }
    }
    ...
    void DrainStderrLoop(CancellationToken ct)
    {
        try
        {
            ...
            while (!ct.IsCancellationRequested)
            {
                ...
                Logger.Debug(new string(buf, 0, n));
            }
        }
        catch { /* ignore */ }
    }
    ...
    {
        EnsureChosenEncoder();
        Logger.Info("[FFMPEG] chosen encoder = " + _chosenGpu);
            ...
            case GpuEnc.AMF:
                {
                    ...
                    if (PixelFormat == InputFormat.NV12)
                    {
                        ...
                        inPart =
                            ...
                            $"-f rawvideo -pix_fmt nv12 -s {{Width}}x{{Height}} -r {{FPS}} -i - ";
                        vfPart = "";
                        Logger.Info("[FFMPEG-AMF] Using NV12 input (zero-copy friendly)");
                    }
                    else
                    {
    ...
    public void Start()
    {
        lock (_sync)
        {
            ...
            Logger.Info("[FFMPEG] exe=" + _ff.StartInfo.FileName);
            _ff.Start();
