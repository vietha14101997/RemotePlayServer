...
public class LibAvEncoderAdapter : ITextureEncoder
{
    ...
    public bool Initialize(int width, int height, int fps, int bitrate, ID3D11Device device, VideoCodec preferredCodec)
    {
        ...
        try
        {
            ...
            Logger.Info($"[LibAvEncoderAdapter] Initializing {width}x{height} @ {fps}fps, {bitrate}kbps, codec={preferredCodec}");
            ...
            {
                Logger.Error("[LibAvEncoderAdapter] LibAvEncoder initialization failed");
                Cleanup();
                return false;
            }

            Logger.Info($"[LibAvEncoderAdapter] Initialized successfully with codec: {_encoder.CurrentCodec}");
            return true;
        }
        catch (Exception ex)
        {
            Logger.Error($"[LibAvEncoderAdapter] Initialize exception: {ex.Message}");
            Cleanup();
            return false;
        }
    }
    ...
    public bool EncodeTexture(ID3D11Texture2D texture, bool forceKeyframe = false)
    {
        ...
        {
            Logger.Error($"[LibAvEncoderAdapter] EncodeTexture exception: {ex.Message}");
            return false;
        }
    }
    ...
    public bool EncodeNV12Bytes(byte[] nv12Bytes, int width, int height, bool forceKeyframe = false)
    {
        ...
        {
            Logger.Error($"[LibAvEncoderAdapter] EncodeNV12Bytes exception: {ex.Message}");
            return false;
        }
    }
    ...
    public bool SetFps(int fps)
    {
        ...
        Logger.Warn($"[LibAvEncoderAdapter] Runtime FPS change not supported (requested: {fps}fps)");
        return false;
    }
    ...
    public void Dispose()
    {
        ...
        Logger.Info("[LibAvEncoderAdapter] Disposed");
    }
}
