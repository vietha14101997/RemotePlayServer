...
internal static class WgcInterop
{
    ...
    public static ID3D11Texture2D? TryGetTextureFast(IDirect3DSurface surface)
    {
        ...
        try
        {
            ...
            if (nativePtr == IntPtr.Zero) { Logger.Error("[WGC] TryGetTextureFast: Failed to get native pointer"); return null; }
            ...
            if (hr == 0 && accessPtr != IntPtr.Zero)
            {
                ...
                if (texPtr != IntPtr.Zero)
                {
                    ...
                    success = true;
                    Logger.Debug("[WGC] TryGetTextureFast: SUCCESS via IDirect3DDxgiInterfaceAccess");
                    return result;
                }
            }
            ...
            if (hr == 0 && texPtr != IntPtr.Zero)
            {
                var result = new ID3D11Texture2D(texPtr);
                success = true;
                Logger.Debug("[WGC] TryGetTextureFast: SUCCESS via direct ID3D11Texture2D QI");
                return result;
            }
            ...
            {
                Logger.Debug("[WGC] TryGetTextureFast: Found IDXGIResource, trying to get texture...");
                ...
                if (hr == 0 && texPtr != IntPtr.Zero)
                {
                    var result = new ID3D11Texture2D(texPtr);
                    success = true;
                    Logger.Debug("[WGC] TryGetTextureFast: SUCCESS via IDXGIResource");
                    return result;
                }
            }
            ...
            {
                Logger.Debug("[WGC] TryGetTextureFast: Found IDXGISurface, trying to get texture...");
                ...
                if (hr == 0 && texPtr != IntPtr.Zero)
                {
                    var result = new ID3D11Texture2D(texPtr);
                    success = true;
                    Logger.Debug("[WGC] TryGetTextureFast: SUCCESS via IDXGISurface");
                    return result;
                }
            }
            
            // Log which interfaces are available for debugging
            Logger.Error($"[WGC] TryGetTextureFast: All methods failed. Surface type: {surface.GetType().FullName}");
            return null;
        }
        catch (Exception ex)
        {
            Logger.Error($"[WGC] TryGetTextureFast exception: {ex.Message}");
            return null;
        }
        finally
        {
    ...
    static SoftwareBitmap CreateBitmapFromSurface(IDirect3DSurface surface)
    {
        ...
        Logger.Error("[WGC] CPU fallback: CreateCopyFromSurfaceAsync timeout");
    ...
    public unsafe static void CopySurfaceToScratch_CPU(IDirect3DSurface surface, byte[] scratch, uint w, uint h, uint strideOut)
    {
        ...
        {
            Logger.Error($"[WGC] CPU fallback async error: {ae.InnerException?.Message ?? ae.Message}");
...
public sealed class WgcCapture : IDisposable
{
    ...
    static IDXGIAdapter1? FindAdapterForMonitor(IntPtr hmon)
    {
        ...
        for (uint ai = 0; ; ai++)
        {
            ...
            for (uint oi = 0; ; oi++)
            {
                ...
                {
                    Logger.Info($"[WGC] Found adapter for monitor: {adapter.Description.Description}");
                    return adapter;
                }
            }
            adapter.Dispose();
    ...
    static (ID3D11Device d3d, ID3D11DeviceContext ctx) CreateD3DDevice(IDXGIAdapter1? adapter = null)
    {
        ...
        if (adapter != null)
        {
            Logger.Info($"[WGC] Creating D3D device on adapter: {adapter.Description.Description}");
        ...
        {
            Logger.Info("[WGC] Creating D3D device on default adapter");
            D3D11.D3D11CreateDevice(
                null,
                DriverType.Hardware,
                DeviceCreationFlags.BgraSupport | DeviceCreationFlags.VideoSupport,
    ...
    void InitCommon(GraphicsCaptureItem item)
    {
        ...
        Logger.Info($"[WGC] Creating FramePool: {size.Width}x{size.Height}");
        ...
        Logger.Info("[WGC] FramePool created, subscribing to FrameArrived...");
        _pool.FrameArrived += OnFrameArrived;
        Logger.Info("[WGC] Creating capture session...");
        _session = _pool.CreateCaptureSession(item);
        _session.IsCursorCaptureEnabled = false;
        Logger.Info("[WGC] Capture session created");
    ...
    public void Start()
    {
        Logger.Info("[WGC] StartCapture() called");
        _session.StartCapture();
        Logger.Info("[WGC] StartCapture() completed - frames should start arriving");
    }

    void OnFrameArrived(Direct3D11CaptureFramePool sender, object args)
    {
        Logger.Debug("[WGC] OnFrameArrived triggered");
        try
        {
            using var frame = sender.TryGetNextFrame();
            if (frame == null) { Logger.Debug("[WGC] TryGetNextFrame returned null"); return; }

            var sz = frame.ContentSize;
            if (sz.Width != _w || sz.Height != _h)
            {
                Logger.Info($"[WGC] Size mismatch: pool={_w}x{_h}, frame={sz.Width}x{sz.Height} - recreating pool");
                ...
                try
                {
                    ...
                    _pool.FrameArrived += OnFrameArrived;
                    Logger.Info($"[WGC] Pool recreated: {_w}x{_h}, event re-subscribed");
                }
                catch (Exception e)
                {
                    Logger.Error("[WGC] Recreate(pool) failed: " + e);
                }
                return; // bỏ frame giao thời
            }

            Logger.Debug($"[WGC] Processing frame: {sz.Width}x{sz.Height}");
            bool fastOK = false;
            ...
            if (surface == null && rawSurface is WinRT.IWinRTObject winrtSurf)
            {
                try
                {
                    ...
                    Logger.Debug("[WGC] Surface cast via MarshalInterface succeeded");
                }
                catch (Exception ex)
                {
                    Logger.Error($"[WGC] Surface cast failed: {ex.Message}");
                }
            }
            
            if (surface == null)
            {
                Logger.Error("[WGC] Failed to get IDirect3DSurface");
                return;
            }
            
            try
            {
                Logger.Debug("[WGC] Trying fast GPU path...");
                using var src = WgcInterop.TryGetTextureFast(surface);
                using (src)
                {
                    if (src != null)
                    {
                        Logger.Debug("[WGC] Got texture, copying to staging...");
                        _ctx.CopyResource(_staging, src);
                        Logger.Debug("[WGC] Mapping staging buffer...");
                        var box = _ctx.Map(_staging, 0, Vortice.Direct3D11.MapMode.Read, Vortice.Direct3D11.MapFlags.None);
                        try
                        {
                            uint srcStride = box.RowPitch;
                            EnsureScratch();
                            Logger.Debug($"[WGC] Copying pixels: srcStride={srcStride}, dstStride={_stride}, h={_h}");
                            unsafe
                            {
                            ...
                            Logger.Debug("[WGC] Pixel copy done");
                        }
                    ...
                    {
                        Logger.Debug("[WGC] TryGetTextureFast returned null");
                    }
                }
            }
            catch (Exception ex) { Logger.Error("[WGC] Fast path failed: " + ex.Message); fastOK = false; }

            if (!fastOK)
            {
                Logger.Debug("[WGC] Using CPU fallback path...");
                WgcInterop.CopySurfaceToScratch_CPU(surface, _scratch, _w, _h, _stride);
                Logger.Debug("[WGC] CPU fallback done");
            }

            Logger.Debug("[WGC] Invoking OnFrame callback...");
            OnFrame?.Invoke(_scratch, (int)_w, (int)_h, (int)_stride);
            Logger.Debug("[WGC] OnFrame callback completed");
        }
        catch (Exception ex)
        {
            Logger.Error("[WGC] OnFrameArrived error: " + ex);
        }
    }
