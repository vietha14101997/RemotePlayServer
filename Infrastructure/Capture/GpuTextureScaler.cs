...
";
        ...
        private void InitializeShader()
        {
            try
            {
                ...
                {
                    Logger.Error("[GpuTextureScaler] Failed to compile shader");
                    return;
                }
                ...
                if (_initialized)
                {
                    Logger.Info($"[GpuTextureScaler] Monitor {_monitorIndex}: Compute shader initialized");
                }
            }
            catch (Exception ex)
            {
                Logger.Error($"[GpuTextureScaler] InitializeShader failed: {ex.Message}");
                _initialized = false;
            }
        }
        ...
        private IntPtr CompileShader(string source, string entryPoint, string target)
        {
            ...
            if (result != 0)
            {
                if (errorBlob != IntPtr.Zero)
                {
                    ...
                    Logger.Error($"[GpuTextureScaler] Shader compile error: {error}");
            ...
            int monitorIndex)
        {
            ...
            try
            {
                ...
                if (_outputTexture == null || _outputWidth != targetWidth || _outputHeight != targetHeight)
                {
                    ...
                    Logger.Info($"[GpuTextureScaler] Monitor {_monitorIndex}: Output texture {targetWidth}x{targetHeight}");
                }
            ...
            {
                Logger.Error($"[GpuTextureScaler] Monitor {_monitorIndex} scale failed: {ex.Message}");
                return null;
            }
        }
