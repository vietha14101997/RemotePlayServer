...
namespace RemotePlayServer.Infrastructure.Network
{
    ...
    public static class UsbTetheringHelper
    {
        ...
        public static UsbTetheringInfo Detect()
        {
            ...
            try
            {
                ...
                foreach (var ni in interfaces)
                {
                    ...
                    if (usbIP != null || (isNdisInterface && ipv4Addresses.Count > 0))
                    {
                        ...
                        Logger.Info($"[USB-Tether] Detected by: {(usbIP != null ? "IP range" : "NDIS interface")}");
                        ...
                        Logger.Info($"[USB-Tether] Found interface: {ni.Name}");
                        Logger.Info($"[USB-Tether] Description: {ni.Description}");
                        Logger.Info($"[USB-Tether] Server IP: {serverIP}");
                        Logger.Info($"[USB-Tether] Gateway (phone): {gateway?.Address}");
                ...
                foreach (var ni in interfaces)
                {
                    ...
                    if (usbIP != null)
                    {
                        ...
                        Logger.Info($"[USB-Tether] Found by IP range: {ni.Name} ({usbIP})");
                        return result;
                    }
                }
            }
            ...
            {
                Logger.Error($"[USB-Tether] Detection error: {ex.Message}");
            }
        ...
        {
            Logger.Info("[USB-Tether] All network interfaces:");
            var interfaces = NetworkInterface.GetAllNetworkInterfaces()
                .Where(ni => ni.OperationalStatus == OperationalStatus.Up)
                .Where(ni => ni.NetworkInterfaceType != NetworkInterfaceType.Loopback);
            ...
            foreach (var ni in interfaces)
            {
                ...
                Logger.Info($"  - {ni.Name}: {ipv4 ?? "no IPv4"} ({ni.Description})");
            }
        }
    }
}
